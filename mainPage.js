const getTextFromSelectorIfExists = (selector, wrapper = document) => wrapper.querySelector(selector)?.textContent;
...
const getTitle = async capsule => {
	...
	const fetchTitleFromUrl = async (constructedUrl, backupHref) =>
		await fetch(constructedUrl)
			...
			.then(async response => (await response.text()) || (await fetch(backupHref).then(response => response.text())))
			.then(text => document.createRange().createContextualFragment(text))
			.then(fragment => getTextFromSelectorIfExists('.hover_title', fragment) || getTextFromSelectorIfExists('title', fragment));

	return fetchTitleFromUrl(url, href);
};

const labelStoreCapsules = async () => {
	document.querySelectorAll('a:not(.gutter_item)[data-ds-appid], a:not(.gutter_item)[data-ds-packageid], a:not(.gutter_item)[data-ds-bundleid]').forEach(capsule => {
		getTitle(capsule)
			.then(title => {
				if (title) {
					const price = getPriceFromCapsule(capsule);
					const platforms = [...capsule.querySelectorAll('.platform_img')].map(getBadgeText).join(', ');
					const topTags = [...capsule.querySelectorAll('.top_tag')].map(tag => tag.textContent).join('');
					const vrOnly = capsule.querySelector('.vr_required');

					setLabel(
						capsule,
						`${title}${price ? ': ' + price : ''}${vrOnly ? ' VR Only.' : ''}${platforms.length ? ' Platforms: ' + platforms + '.' : ''}${
...
const labelSpotLights = async () => {
	for (const spotlight of document.querySelectorAll(':not(a)[data-ds-appid], :not(a)[data-ds-packageid], :not(a)[data-ds-bundleid]')) {
...
// TODO: remove this and import from badges.js
	...
	document
		...
		.forEach(weekLongDealsIdentifier =>
			setLabel(
				...
				`Week long deals: ${weekLongDealsIdentifier.querySelector('.spotlight_count').textContent} on sale this week. ${
					weekLongDealsIdentifier.parentNode.querySelector('.spotlight_weeklong_subtitle')?.textContent
				}`
			)
		);
};
...
window.addEventListener('load', accessify);
